...
class QuoteApp:
    ...
    def setup_ui(self):
        ...
        self.quote_label = tk.Label(self.root, text="Нажмите кнопку ниже",
                                    wraplength=500, font=("Arial", 12, "italic"), fg="blue")
        self.quote_label.pack(pady=20)

        # Кнопка генерации (Пункт 2)
        btn_gen = tk.Button(self.root, text="Сгенерировать цитату", command=self.generate_quote, bg="#e1e1e1")
        btn_gen.pack(pady=5)
        ...
        tk.Label(add_frame, text="Текст:").grid(row=0, column=0)
        ...
        self.entry_author = tk.Entry(add_frame, width=50)
        self.entry_author.grid(row=1, column=1, padx=5, pady=2)

        tk.Label(add_frame, text="Тема:").grid(row=2, column=0)
        self.entry_theme = tk.Entry(add_frame, width=50)
        self.entry_theme.grid(row=2, column=1, padx=5, pady=2)

        btn_add = tk.Button(add_frame, text="Добавить в базу", command=self.add_custom_quote)
        ...
        tk.Label(filter_frame, text="Поиск в истории (Автор/Тема):").pack(side=tk.LEFT)
        ...
        self.history_listbox = tk.Listbox(self.root, width=80, height=8)
